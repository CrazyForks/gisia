...
class ProjectFeature < ApplicationRecord
  ...
  EXPORTABLE_FEATURES = (FEATURES - %i[security_and_compliance pages metrics_dashboard]).freeze
      ...
      where(
        "#{column} IS NULL OR #{column} IN (:public_visible) OR (#{column} = :private_visible AND EXISTS (:authorizations))",
        {
          ...
          authorizations: user.authorizations_for_projects(min_access_level: min_access_level,
            related_project_column: 'project_features.project_id')
        }
      )
    else
  ...
  def package_registry_access_level=(value)
    super.tap do
      project.packages_enabled = package_registry_access_level != DISABLED if project
    end
  end
  ...
  def set_pages_access_level
    self.pages_access_level ||= if ::Gitlab::Pages.access_control_is_forced?
                                  ...
                                  project&.public? ? ENABLED : PRIVATE
                                end
  end
  ...
  def repository_children_level
    validator = ->(field) do
      level = public_send(field) || ENABLED
